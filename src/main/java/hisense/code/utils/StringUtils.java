...
public class StringUtils {
	...
	// 2016-09-21 add by mafei 台账统计小数点后两位问题
	public static String formatDouble2(Object value) {
		if (Check.isDouble(value + "")) {
			DecimalFormat df = new DecimalFormat("#.00");
			String dv = df.format(Double.valueOf(value + ""));
			return dv;
	...
	public static String createJson(Map<String, Object> info) {
		...
		if (info != null && info.size() > 0) {
			...
			for (String key : info.keySet()) {
				...
				if (value != null
						&& (value.toString().startsWith("[") || value
								.toString().startsWith("{")))
					json += "\"" + key + "\":" + value;
				else {
					json += "\"" + key + "\":\"" + value + "\"";
	...
	public static boolean checkPlate(String plate) {
		...
		String[] province = new String[] { "京", "沪", "浙", "苏", "粤", "鲁", "晋",
				"冀", "豫", "川", "渝", "辽", "吉", "黑", "皖", "鄂", "湘", "赣", "闽",
				"陕", "甘", "宁", "蒙", "津", "贵", "云", "桂", "琼", "青", "新", "藏",
				"港", "澳", "使", "军", "空", "海", "北", "沈", "兰", "济", "南", "广",
				"成", "WJ", "警", "消", "边", "水", "电", "林", "通" };
		Pattern p = Pattern
				.compile("^[\\u4e00-\\u9fa5]{1}[A-Z_0-9]{5}[A-Z_0-9_\\u4e00-\\u9fa5]$|^WJ\\d{7}$");
	...
	public static Double[] processBonus(Double totalMoney, Integer personNum) {
		if (totalMoney == null || totalMoney == 0 || personNum == null
				|| personNum == 0)
			return null;
	...
	public static void main(String[] args) {
		...
		/*
		 * try { String pass = "guilin0316"; pass =StringUtils.MD5(pass); pass =
		 * StringUtils.MD5(pass +"zldtingchebao201410092009");
		 * System.out.println(pass); } catch (Exception e) { // TODO
