...
public class TimeTools {
	...
	// 时间格式数组
	private static String[] formatArray = { "yyyy-MM-dd", "yyyy-MM-dd HH:mm", "yyyy-MM-dd HH:mm:ss", "yy-MM-dd HH:mm",
			"yyyyMMdd HH:mm", "yyyy-MM-dd HH" };
	...
	public static String checkMounth(String endDateSelect) {
		...
		if (mounth.equals("1") || mounth.equals("3") || mounth.equals("5") || mounth.equals("7") || mounth.equals("8")
				|| mounth.equals("10") || mounth.equals("12"))
			return "-31";
	...
	// 计算一周前的时间
	public static Long getToWeekBeginTime() {
		Date d = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String today = sdf.format(new Date(d.getTime() - 7 * 24 * 60 * 60 * 1000));
		today = today.substring(0, 10) + " 00:00:00";
		return getStrDateToSecond(today);
	}

	/*
	 * 获得传入日期的零点秒值 miliseconds毫秒数
	 */
	 ...
	 * @param week
	 *            周期
	 * @return 时间段内所有天数对应日期的List集合
	 */
	public static List<String> getDay(String time1, String time2, String week) {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar c = Calendar.getInstance();
		Date beginTime = null;
		List<String> dateList = new ArrayList<String>(); // 存放每天日期的集合
		String startTime = time1;
		String endTime = time2;
		int dayIndex = getAllDays(startTime, endTime);// 时间段内总天数
		int weekIndex = Integer.parseInt(week);
		if (weekIndex == 0) { // 获取全部日期
			try {
	...
	public static int getAllDays(String startTime, String endTime) {
		...
		try {
			...
			days = (int) ((stopTime.getTime() - beginTime.getTime()) / (1000 * 3600 * 24));
			return days;
		} catch (ParseException e) {
			e.printStackTrace();
		}
	 ...
	 * 
	 */
	public static List<String> getYesterdayAndHistoryDays() {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		...
		String history = sdf.format(c.getTime());
		dList.add(yesterday);
		dList.add(history);
		return dList;
	}
	 ...
	 * @return
	 */
	public static List<String> getTodayAndMonthFirst() {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		List<String> tList = new ArrayList<String>();
		Calendar c = Calendar.getInstance();
		c.add(Calendar.DATE, 0);
		String today = sdf.format(c.getTime());
		String month = today.substring(0, today.length() - 2);
		month += "01";
		tList.add(today);
		tList.add(month);
		return tList;
